...
const ItemDashboard = () => {
  ...
  useEffect(() => {
    const fetchItemsAndData = async () => {
      ...
      try {
        const itemsResponse = await axios.get(`https://shahil14.pythonanywhere.com/api/items/?${params.toString()}`, {
            headers: {
        ...
        if(!categories.length){
          const categoriesResponse = await axios.get('https://shahil14.pythonanywhere.com/api/categories/', {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
              },
          });
        ...
        if(!tags.length){
          const tagsResponse = await axios.get('https://shahil14.pythonanywhere.com/api/tags/', {
              headers: {
                Authorization: `Bearer ${localStorage.getItem('token')}`,
              },
          });
  ...
  const handleNewCategorySubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://shahil14.pythonanywhere.com/api/categories/', { name: newCategoryName }, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
  ...
  const handleNewItemSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://shahil14.pythonanywhere.com/api/items/', sendNewItem, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
        },
      });
